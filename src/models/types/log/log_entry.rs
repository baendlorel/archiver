...
impl Tablify for LogEntry {
    fn to_table_row(&self) -> crate::misc::console::table::TableRow {
        ...
        let avid = match (archive_id.is_empty(), vault_name.is_empty()) {
            ...
            (false, true) => format!("{}", archive_id.styled_id(),),
    ...
    fn display_vertically(&self) {
        ...
        let archive_ids = join_archive_ids(&self.archive_ids);
        let vault_ids = join_vault_ids(&self.vault_ids);
        let rows = vec![
            kv_row!("Id", id),
            ...
            kv_row!("Archive Ids", format!("[{}]", archive_ids)),
            kv_row!("Vaults", format!("[{}]", vault_ids)),
            kv_row!(
                "Remark",
...
fn join_vault_ids(ids: &Option<Vec<u32>>) -> String {
    if let Some(ids) = ids {
        ids.iter()
            .map(|n| format!("{}", vault::get_name_styled(*n)))
            .collect::<Vec<String>>()
            .join(", ")
    } else {
        String::new()
