...
macro_rules! impl_get_type_for_types {
    ($variant:path; $($t:ty),+) => {
        $(
            impl GetType for $t {
                fn get_type(&self) -> VarType {
...
impl_get_type_for_types!(VarType::Bool; bool);

impl_get_type_for_types!(VarType::String; String, &str);

// 为所有数字类型（整数和浮点数）实现 GetType
impl_get_type_for_types!(VarType::Int;
