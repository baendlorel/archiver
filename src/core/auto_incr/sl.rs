...
use crate::models::{error::ArchiverResult, serde_custom::SerdeJson, types::AutoIncrVars};

pub fn save(auto_incr: &AutoIncrVars) -> ArchiverResult<()> {
    let json = as_fatal!(auto_incr.to_formatted_string())?;
    as_fatal!(fs::write(paths::AUTO_INCR_FILE_PATH.as_path(), json))?;
    Ok(())
}

pub fn load() -> ArchiverResult<AutoIncrVars> {
